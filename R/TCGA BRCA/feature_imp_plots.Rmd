...
Plotting
```{r}
ggplot(ranks, aes(x=anova, y=lime, colour=Datatype))+
  geom_point()+
  labs(x="ANOVA Ranks", y="LIME Ranks")
```
...
Plotting
```{r}
ggplot(ranks, aes(x=lime, y=svm_lime, colour=Datatype))+
  geom_point()+
  labs(x="GCN Lime Ranks", y="SVM LIME Ranks")
```

## Correlation plot
```{r}
matrix <- values[c("shapley_values", "aggregate.x", "f_value", "aggregate.y")]
matrix <- rename(matrix, "gcn_shap"="shapley_values", "gcn_lime"="aggregate.x", "anova"="f_value", "svm_lime"="aggregate.y")
...
shap_all_int <- read_csv("/data/users/bs16b001/logs/20210217/shap_1.csv", 
    col_types = cols(features = col_skip())) %>% rename("features"="features_1")
...
```{r}
enriched_pathways <- fgseaRes_gcnlime$pathway[fgseaRes_gcnlime$padj < 0.05]
...
temp <- sapply(selected_pathways_list, function(x) sapply(selected_pathways_list, function(y) {
    length(intersect(x,y))/length(union(x,y))
  }))

# temp[,"GO_NEGATIVE_REGULATION_OF_PEPTIDE_SECRETION"]
```
