...
if __name__ == "__main__":    
    ...
    data_list, labels, patient_id, feat_name_list = load_preproc_data(mrna_preproc_csv, meth_preproc_csv, mirna_preproc_csv, mirna_mrna_csv, meta_csv, label_dict)
    ...
    test_inverval = 50
    lr = 1e-3
    dim_list = [200,200,200,134] # Input dimension of GCN
    dim_he_list = [200,200,200,134] # Hidden layer dimension of GCN
    ...
    data_tr_list.append(mirna_interaction[:,:200])

    # For training and testing data
    mirna_interaction = findInteractions(data_trte_list[2])
    if cuda:    
        mirna_interaction.cuda()
    data_trte_list.append(mirna_interaction[:,:200])
    # End Mods
    '''
    ...
    for epoch in range(num_epoch+1):
        """ Train one epoch
        """
        tr_loss_dict = train_1_epoch(data_tr_list, adj_tr_list, labels_tr_tensor, 
        ...
        if epoch % test_inverval == 0:
            ...
            if VERBOSE >= 1 or (epoch == num_epoch):
                ...
                print("Test ACC: {:.4f}".format(te_acc))
                print("Test F1: {:.4f}".format(te_f1))
                print()
    ...
    plt.plot(x,losses[3], color='r', label="GCN_miRNA_mrna_interaction")
    plt.plot(x,losses[4], color='c', label="VCDN")
    plt.yscale("log")
    plt.xlabel("Epochs")
    plt.ylabel("Loss (on log scale)")
    plt.title("MORONET Losses - BRCA Dataset - With miRNA-mRNA Interaction")
    plt.legend()
    #plt.show()
    time = str(datetime.datetime.now())
    plt.savefig("Losses_miRNA-mRNA_"+time+".png")
    ...
    plt.title("MORONET Test Evaluation - BRCA Dataset - With miRNA-mRNA Interaction")
    plt.legend()
    #plt.show()
    plt.savefig("Test_Scores_miRNA-mRNA_"+time+".png")
