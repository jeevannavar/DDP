...
RUN_TITLE = "ANOVA top 1000 features selected greedily with correlation below 0.8 \nThis run includes only 9 GCNs - mrna, meth, mirna, mrna_mrna, meth_meth, mirna_mirna, mrna_meth, meth_mirna, mirna_mrna"
RUN_TITLE_SHORT = "brca_all_int"
...
load_list = [mrna, meth, mirna, mrna_mrna, meth_meth, mirna_mirna, mrna_meth, meth_mirna, mirna_mrna, meta_csv, trte_partition_file]
GCN_names = ["mRNA","methylation","miRNA", "mrna_mrna", "meth_meth", "mirna_mirna", "mrna_meth", "meth_mirna", "mirna_mrna"]

COMBINER = "FullyConnected" # Can take values "VCDN", "FullyConnected", or False.
...
for i in range(1,REPEATS+1):
    ...
    print("Run {}".format(i))
    print("-"*100)
    losses_df, metrics_df, feature_imp = train_model(load_list=load_list, label_dict=label_dict, GCN_names=GCN_names, COMBINER=COMBINER,
    ...
    for key in feature_imp.keys():
        feature_imp[key].to_csv("{}_{}.csv".format(key,i), index_label="features")
