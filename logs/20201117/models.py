...
def init_model_dict(num_class, dim_list, dim_he_list, dim_hc, gcn_dopout=0.5):
    """
    ...
    for i in range(len(dim_list)):
        encoder = f"E{i+1}"
        model_dict[encoder] = GCN_E_2(dim_list[i], dim_he_list[i], dim_he_list[i], gcn_dopout)
        classifier = f"C{i+1}"
...
def init_optim(model_dict, dim_list, lr=1e-3):
    """
    ...
    for i in range(len(dim_list)):
        classifier = f"C{i+1}"
        encoder = f"E{i+1}"
        optim_dict[classifier] = torch.optim.Adam(list(model_dict[encoder].parameters())+list(model_dict[classifier].parameters()), lr=lr)
    optim_dict["C"] = torch.optim.Adam(model_dict["C"].parameters(), lr=lr)
