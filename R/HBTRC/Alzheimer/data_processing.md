...
labels <- read_csv("labels_AD.csv")

# Getting the training indices
trte <- file("trte_partition_AD.txt", open = "r")
...
write_csv(data, "cerebellum_AD.csv")
```
...
data <- rbind(data_train, data_test)
data <- data[match(labels$patient_id, data$patient_id), ]
write_csv(data, "visualCortex_AD.csv")
```
...
data <- rbind(data_train, data_test)
data <- data[match(labels$patient_id, data$patient_id), ]
write_csv(data, "prefrontalCortex_AD.csv")
```
...
``` r
labels <- read_csv("labels_braak.csv")
...
# Getting the training indices
trte <- file("trte_partition_braak.txt", open = "r")
lines <- readLines(trte)
close(trte)
train_idx <- unlist(strsplit(lines[2], ","))
test_idx <- unlist(strsplit(lines[4], ","))
...
data <- rbind(data_train, data_test)
data <- data[match(labels$patient_id, data$patient_id), ]
write_csv(data, "cerebellum_braak.csv")
```
...
data <- rbind(data_train, data_test)
data <- data[match(labels$patient_id, data$patient_id), ]
write_csv(data, "visualCortex_braak.csv")
```
...
data <- rbind(data_train, data_test)
data <- data[match(labels$patient_id, data$patient_id), ]
write_csv(data, "prefrontalCortex_braak.csv")
```
