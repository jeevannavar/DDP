...
RUN_TITLE = "No Pre-selection of Features"
RUN_TITLE_SHORT = "complete"
print(RUN_TITLE)
print("This run includes 3 GCNs - mRNA, DNA methylation, miRNA","\n")
# assign GPU number
CUDA_DEVICE = 0
...
if __name__ == "__main__":    
    ...
    ground_truth = labels_trte[trte_idx["te"]]
    accuracies = label_specific_acc(ground_truth, te_prob.argmax(1))
    counter = Counter(ground_truth)
    print("Here, we have the label specific accuracies:\n")
    print("Label", "\t  ", "Occurrence", "\t", "Prediction Accuracy")
    print("-"*50)
    for each in label_dict:
        print(each, "\t\t", "{:.4f}".format(counter[label_dict[each]]/len(ground_truth)), "\t\t", accuracies[label_dict[each]])
