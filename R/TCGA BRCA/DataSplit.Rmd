...
pam50 <- read_csv("/data/users/bs16b001/R/pam50.csv") %>%
          mutate(pam50, Sample_ID = sub(pattern, "\\1", Sample_ID)) %>%
...
## The distribution
```{r}
pam50$Category <- ifelse(pam50$patient_id %in% train_idx, "Train", "Test")

pam50 %>%
  ggplot(aes(x=cancer_subtype, group=Category, fill=Category))+
  geom_bar(position="dodge")+
  labs(x="Cancer subtype", y="Number of samples", title = "Sample distribution")

(Number_of_train_samples = length(train_idx))
(Number_of_test_samples = nrow(pam50) - length(train_idx))
