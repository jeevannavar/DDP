...
def train_1_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict):
    """
    ...
    classifiers = ["C%d" % i for i in range(1,len(data_list)+1)]
    ...
    c = model_dict["C"](views)    
    c_loss = torch.mean(torch.mul(criterion(c, label),sample_weight))*C_loss_weight["C"]
    c_loss.backward()
    optim_dict["C"].step()
...
def test_VCDN(data_list, adj_list, te_idx, model_dict):
    """ 
    Test the model
    Input:
        data_list   -> list of GCN input tensors, for ex: data_list[0] is view 1 data
